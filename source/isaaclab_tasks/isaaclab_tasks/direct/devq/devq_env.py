...
class DevqEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: DevqFlatEnvCfg | DevqRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 添加速度历史缓冲区
        self.velocity_window_size = 2  # 使用10步来计算平均速度
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.cat(
            [
                ...
                velocity_commands,    # [3]
                imu_ang_vel,         # [3]
                imu_lin_acc,     # [3] - Now correctly using quaternion
                proj_gravity,        # [3]
                joint_positions,     # [12]
                joint_velocities,    # [12]
                actions,            # [12]
        ...
        observations = {"policy": obs}  # Total dimensions: 49 (increased by 1 due to quaternion)
        return observations
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        lin_vel_error_mapped = torch.exp(-lin_vel_error / 0.2)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
       ...
       # Sample new commands with positive X velocity
        ...
        # y方向依然在 [-1.0, 1.0] 范围内
        new_commands[:, 1] = torch.rand_like(new_commands[:, 1]) * 1.4-0.7# x velocity (0 to 1)
        # z方向依然在 [-1.0, 1.0] 范围内
        new_commands[:, 2] = torch.rand_like(new_commands[:, 2]) *1.4-0.7# x velocity (0 to 1)
