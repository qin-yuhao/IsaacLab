...
class MitFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    #24
    num_steps_per_env = 24 #* 2
    max_iterations =4000
    save_interval = 50
    experiment_name = "Mitw_flat_direct"
    empirical_normalization = False
    policy = RslRlPpoActorCriticCfg(
        #class_name="ResidualActorCritic",
        init_noise_std=1.0,
        actor_hidden_dims=[128 for _ in range(3)],
        critic_hidden_dims=[128 for _ in range(3)],
        activation="elu",
    )
...
class MitRoughPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    save_interval = 50
    experiment_name = "mitw_rough_direct"
    empirical_normalization = False
