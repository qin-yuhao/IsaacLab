...
from .big_env_cfg import BigFlatEnvCfg, BigRoughEnvCfg


class BigEnv(DirectRLEnv):
    cfg: BigFlatEnvCfg | BigRoughEnvCfg

    def __init__(self, cfg: BigFlatEnvCfg | BigRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 命令重新采样相关变量
        self._command_resample_time_min = 1.0  # 最小重新采样间隔（秒）
    ...
    def _setup_scene(self):
        ...
        if isinstance(self.cfg, BigRoughEnvCfg):
        ...
        obs = torch.cat(
            [
                ...
                for tensor in (
                    ...
                    joint_velocities,    # [12]
                    actions,            # [12]
                    height_data,         # [N, num_rays] or None
                )
    ...
    def _compute_specific_joint_vel_penalty(self) -> torch.Tensor:
        """针对特定关节的速度惩罚，可以分别设置不同权重"""
        joint_velocities = self._robot.data.joint_vel  # [N, 12]
        
        # 假设关节排列：前腿(0-5)，后腿(6-11)
        # 可以根据实际机器人关节配置调整索引
        front_legs_ids = [0, 1, 2, 3, 4, 5]  # 前腿关节
        rear_legs_ids = [6, 7, 8, 9, 10, 11]  # 后腿关节
        
        # 分别计算前腿和后腿的速度惩罚
        front_leg_penalty = torch.sum(torch.square(joint_velocities[:, front_legs_ids]), dim=1)
        rear_leg_penalty = torch.sum(torch.square(joint_velocities[:, rear_legs_ids]), dim=1)
        
        # 可以设置不同的权重
        front_weight = 1.0
        rear_weight = 1.0
        
        return front_weight * front_leg_penalty + rear_weight * rear_leg_penalty
