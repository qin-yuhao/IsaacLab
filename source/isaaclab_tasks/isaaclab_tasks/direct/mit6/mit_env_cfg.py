...
class EventCfg:
    ...
    push_robot = EventTerm(
        ...
        mode="interval",
        interval_range_s=(8, 10.0),
        params={"velocity_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5) ,"z": (-0.5, 0.5)}}, #,,"roll": (-1,1),"pitch": (-1,1), "yaw": (-1, 1),"z": (-0.5, 0.5)
    )
...
class MitFlatEnvCfg(DirectRLEnvCfg):
    ...
    action_scale = [0.25 for _ in range(12)]  # scale for each action
    #action_scale = [0.25, 0.25, 0.25, 0.25, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]  # scale for each action
    ...
    joint_torque_reward_scale = -1e-5  # Penalty for joint torques
    joint_accel_reward_scale = -2.5e-7
    action_rate_reward_scale = -0.1  # Penalty for action rate changes
    ...
    height_tracking_scale =1
    long_contact_penalty_scale = -0# 长时间接触地面时的惩罚
    joint_direction_change_scale = -0.0 # 惩罚关节方向的变化    joint_pos_deviation_scale = -0.0
    joint_power_reward_scale: float = -0.0
    joint_pos_deviation_scale = -0.0 # 惩罚关节位置偏离
      # JAX style reward scales
    # feet_slip_penalty_scale: float = -0.5  # 脚滑移惩罚权重
    # feet_clearance_reward_scale: float = -1  # 脚离地高度奖励权重  
    # feet_height_reward_scale: float = -0.5 # 摆动脚高度奖励权重
    # stand_still_penalty_scale: float = -2  # 站立静止时偏离默认姿态的惩罚权重
