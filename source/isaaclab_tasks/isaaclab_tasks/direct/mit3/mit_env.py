...
class MitEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MitFlatEnvCfg | MitRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._joint_pos_randomization_range = 0.2  # 关节位置随机化范围（弧度）
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        rewards = {
            ...
            "joint_vel_l2": joint_vel_penalty * self.cfg.joint_vel_reward_scale * self.step_dt,
            "stand_still_reward": stand_still_reward * self.cfg.stand_still * self.step_dt,  # 使用固定的奖励系数0.5
            "track_height_exp": height_tracking_reward *self.cfg.height_tracking_scale * self.step_dt,  # 高度跟踪奖励
    ...
    def _update_command_resample(self):
        ...
        if torch.any(resample_mask):
            ...
            if torch.rand(1).item() < zero_prob:
                ...
                self._commands[resample_env_ids, 0] = torch.zeros_like(self._commands[resample_env_ids, 0]).uniform_(-2, 2)
            
            # Y轴: -1 到 1，但有额外概率采样0
            if torch.rand(1).item() < zero_prob:
                self._commands[resample_env_ids, 1] = 0.0
            else:
                self._commands[resample_env_ids, 1] = torch.zeros_like(self._commands[resample_env_ids, 1]).uniform_(-0, 0)
            
            # Z轴(角速度): -1.0 到 1.0，但有额外概率采样0
            if torch.rand(1).item() < zero_prob:
                self._commands[resample_env_ids, 2] = 0.0
            else:
                self._commands[resample_env_ids, 2] = torch.zeros_like(self._commands[resample_env_ids, 2]).uniform_(-0, 0)
    ...
    def _compute_height_tracking_reward(self) -> torch.Tensor:
        """计算高度跟踪奖励：机器人实际高度与目标高度的匹配程度
        
        Returns:
            torch.Tensor: 高度跟踪奖励值
        """
        # 获取机器人当前高度
        current_height = self._robot.data.root_pos_w[:, 2]  # Z轴坐标
        
        # 获取目标高度命令
        target_height = self._commands[:, 3]  # 目标高度在命令的第4个位置
        
        # 计算高度误差
        height_error = torch.abs(current_height - target_height)
        
        # 使用指数衰减奖励：误差越小奖励越高
        # 当误差为0时奖励为1，误差越大奖励越接近0
        height_reward = torch.exp(-height_error / 0.1)  # 0.1是衰减系数，可调整
