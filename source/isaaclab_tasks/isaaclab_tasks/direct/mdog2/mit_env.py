...
class MitEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MitFlatEnvCfg | MitRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # X/Y linear velocity, yaw angular velocity, pitch, roll, and height commands
        ...
        self._episode_sums = {
            ...
            for key in [
                ...
                "track_ang_vel_z_exp",
                "track_pitch_exp",  # 新增：pitch角度跟踪奖励
                "track_roll_exp",   # 新增：roll角度跟踪奖励
                ...
                "feet_tangential_force",  # 新增：脚部切向力惩罚
            ]
        }
        ...
        # 正站立关节位置（默认位置的偏移值）
        self.stand_joint_pos = {
            ".*L_hip_joint": -0.1,   # 正站立时左髋关节
            ".*R_hip_joint": 0.1,    # 正站立时右髋关节
            "F[L,R]_thigh_joint": -0.4,  # 正站立时前腿大腿关节
            "R[L,R]_thigh_joint": 0.4,   # 正站立时后腿大腿关节
            "F[L,R]_calf_joint": -1.1,   # 正站立时前腿小腿关节
            "R[L,R]_calf_joint": -1.1,   # 正站立时后腿小腿关节
        }
        
        # 倒立关节位置（正站立基础上的偏移量）
        self.inv_stand_joint_pos = {
            ".*L_hip_joint": 0.2,    # 0.1 - (-0.1) = 0.2
            ".*R_hip_joint": -0.2,   # -0.1 - 0.1 = -0.2  
            "F[L,R]_thigh_joint": -170.0 * 3.14 / 180.0 + 0.4,  # -2.967 + 0.4 = -2.567
            "R[L,R]_thigh_joint": -190.0 * 3.14 / 180.0 - 0.4,  # -3.320 - 0.4 = -3.720
            "F[L,R]_calf_joint": 50.0 * 3.14 / 180.0 + 1.1,     # 0.872 + 1.1 = 1.972
            "R[L,R]_calf_joint": 50.0 * 3.14 / 180.0 + 1.1,     # 0.872 + 1.1 = 1.972
        }
    ...
    def _get_observations(self) -> dict:
        ...
        # Commands (no noise) - 包含6个命令：x_vel, y_vel, yaw_vel, pitch, roll, height
        velocity_commands = self._commands[:, :3]  # [3] - x_vel, y_vel, yaw_vel
        orientation_commands = self._commands[:, 3:5]  # [2] - pitch, roll
        height_command = self._commands[:, 5:6]    # [1] - target height
        ...
        obs = torch.cat(
            [
                tensor
                for tensor in (
                    velocity_commands,    # [3] - x_vel, y_vel, yaw_vel
                    orientation_commands, # [2] - pitch, roll
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # pitch angle tracking (从重力向量计算pitch角度)
        # 使用重力投影向量计算更稳健的pitch和roll角度
        proj_gravity = self._robot.data.projected_gravity_b  # [N, 3]
        
        # 确保重力向量归一化
        gravity_norm = torch.norm(proj_gravity, dim=1, keepdim=True)
        gravity_normalized = proj_gravity / (gravity_norm + 1e-6)
        
        # pitch = arcsin(-gravity_x)，但要考虑符号
        current_pitch = torch.asin(torch.clamp(-gravity_normalized[:, 0], -0.99, 0.99))
        pitch_error = torch.square(self._commands[:, 3] - current_pitch)
        pitch_error_mapped = torch.exp(-pitch_error / 0.25)
        
        # roll角度计算：使用重力向量在YZ平面的投影
        # 当机器人水平时，gravity_z应该是-1，gravity_y应该是0
        # roll = atan2(gravity_y, -gravity_z)，注意这里gravity_z取负号
        current_roll = torch.atan2(gravity_normalized[:, 1], -gravity_normalized[:, 2])
        roll_error = torch.square(self._commands[:, 4] - current_roll)
        roll_error_mapped = torch.exp(-roll_error / 0.25)
        
        # body height tracking
        current_height = self._robot.data.root_pos_w[:, 2]
        height_error = torch.square(self._commands[:, 5] - current_height)
        ...
        # feet tangential force penalty
        feet_tangential_force_penalty = self._compute_feet_tangential_force_penalty()
        
        rewards = {
            "track_lin_vel_xy_exp": lin_vel_error_mapped * self.cfg.lin_vel_reward_scale * self.step_dt,
            "track_ang_vel_z_exp": yaw_rate_error_mapped * self.cfg.yaw_rate_reward_scale * self.step_dt,
            "track_pitch_exp": pitch_error_mapped * self.cfg.pitch_reward_scale * self.step_dt,
            "track_roll_exp": roll_error_mapped * self.cfg.roll_reward_scale * self.step_dt,
            ...
            "feet_tangential_force": feet_tangential_force_penalty * self.cfg.feet_tangential_force_reward_scale * self.step_dt,  # 脚部切向力惩罚
        }
    ...
    def _compute_feet_tangential_force_penalty(self) -> torch.Tensor:
        """计算脚部切向力惩罚，防止脚部在障碍物边缘滑动
        
        切向力是指与重力方向垂直的接触力分量。当脚部在障碍物边缘时，
        会产生较大的切向力，这可能导致脚部卡住或滑动。
        
        Returns:
            torch.Tensor: 脚部切向力惩罚值 [N]
        """
        # 获取接触力数据
        net_contact_forces = self._contact_sensor.data.net_forces_w_history
        ...
        # 获取脚部接触力 [N, history_length, num_feet, 3]
        feet_contact_forces = net_contact_forces[:, -1, self._feet_ids, :]  # [N, num_feet, 3]
        
        # 计算接触力的法向和切向分量
        # 假设重力方向为负Z轴，法向力是Z轴分量，切向力是XY分量
        tangential_forces = feet_contact_forces[:, :, :2]  # XY分量 [N, num_feet, 2]
        normal_forces = feet_contact_forces[:, :, 2:3]     # Z分量 [N, num_feet, 1]
        
        # 计算切向力的大小
        tangential_force_magnitude = torch.norm(tangential_forces, dim=2)  # [N, num_feet]
        normal_force_magnitude = torch.abs(normal_forces.squeeze(-1))      # [N, num_feet]
        
        # 只对有显著法向力的脚计算切向力惩罚（接触阈值）
        contact_threshold = 1.0  # 最小接触力阈值
        contact_mask = normal_force_magnitude > contact_threshold  # [N, num_feet]
        
        # 直接使用切向力的大小作为惩罚，设置阈值
        tangential_force_threshold = 5.0  # 切向力阈值（N）
        
        # 计算超出阈值的切向力惩罚
        excessive_tangential_force = torch.clamp(
            tangential_force_magnitude - tangential_force_threshold, min=0.0
        )
        
        # 对每只脚的惩罚进行加权（只对接触的脚计算）
        feet_penalty = excessive_tangential_force * contact_mask.float()
        
        # 返回所有脚的总惩罚
        total_penalty = torch.sum(feet_penalty, dim=1)  # [N]

        total_penalty = torch.square(total_penalty)  # 使用平方函数来计算脚部切向力惩罚
    ...
    #     # 判断是否倒地
    #     fallen_threshold = 150.0  # 超过90度认为倒地
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # 随机决定每个环境的初始姿态：50%概率正站立，50%概率倒立
        random_choice = torch.rand(len(env_ids), device=self.device)
        upright_mask = random_choice < 1  # 50%概率选择正站立
        inverted_mask = ~upright_mask       # 50%概率选择倒立
        
        # Reset robot state - 根据随机选择应用不同的关节位置
        joint_pos = self._randomized_default_joint_pos[env_ids].clone()
        
        # 为正站立的环境应用正站立关节位置
        if torch.any(upright_mask):
            upright_env_ids = env_ids[upright_mask]
            joint_pos[upright_mask] = self._apply_joint_positions_from_config(upright_env_ids, self.stand_joint_pos)
            #print(f"🤖 环境 {upright_env_ids.cpu().numpy()} 初始化为正站立姿态")
        
        # 为倒立的环境应用倒立关节位置
        if torch.any(inverted_mask):
            inverted_env_ids = env_ids[inverted_mask]
            joint_pos[inverted_mask] = self._apply_joint_positions_from_config(inverted_env_ids, self.inv_stand_joint_pos)
            #print(f"🙃 环境 {inverted_env_ids.cpu().numpy()} 初始化为倒立姿态")
        ...
        if torch.any(inverted_mask):
            # 倒立环境的根状态需要翻转
            inverted_root_state = default_root_state[inverted_mask].clone()
            
            # 设置倒立的四元数 (绕X轴旋转180度)
            # 四元数 [w, x, y, z] 表示绕X轴旋转180度: [0, 1, 0, 0]
            inverted_root_state[:, 3] = 0.0  # w
            inverted_root_state[:, 4] = 1.0  # x
            inverted_root_state[:, 5] = 0.0  # y
            inverted_root_state[:, 6] = 0.0  # z
            
            # 提高倒立机器人的初始高度，避免身体接触地面
            #inverted_root_state[:, 2] += 0.5  # 在Z方向增加0.5米高度
            
            # 将修改后的根状态写回
            default_root_state[inverted_mask] = inverted_root_state
    ...
    def _update_command_resample(self):
        """更新命令重新采样逻辑，每3-10秒随机重新采样命令
        命令包括：X、Y线速度，Z角速度，pitch角度，roll角度，高度目标
        """
        # 更新命令计时器
        ...
        if torch.any(resample_mask):
            ...
            # 为前3个命令增加采样0值的概率
            zero_prob = 0.1  # 10%的概率采样0值
            
            # X轴线速度: -1 到 1，但有额外概率采样0
            ...
            # pitch角度目标: -0.3 到 0.3 弧度 (约 ±17度)
            self._commands[resample_env_ids, 3] = torch.zeros_like(self._commands[resample_env_ids, 3]).uniform_(-0.3, 0.3)
            
            # roll角度目标: -0.3 到 0.3 弧度 (约 ±17度)
            self._commands[resample_env_ids, 4] = torch.zeros_like(self._commands[resample_env_ids, 4]).uniform_(-0.3, 0.3)
            
            # 高度目标: 0.15 到 0.4
            self._commands[resample_env_ids, 5] = torch.zeros_like(self._commands[resample_env_ids, 5]).uniform_(0.15, 0.4)
            
            # 重置这些环境的命令计时器
            self._command_timer[resample_env_ids] = 0.0
            
            # 为这些环境设置新的重新采样间隔时间（3-10秒随机）
            self._command_timeout[resample_env_ids] = torch.zeros_like(self._command_timeout[resample_env_ids]).uniform_(
                self._command_resample_time_min, self._command_resample_time_max
            )
    
    def _apply_joint_positions_from_config(self, env_ids: torch.Tensor, joint_config: dict) -> torch.Tensor:
        """将关节位置配置应用到指定环境
        
        Args:
            env_ids: 环境ID
            joint_config: 关节位置配置字典，包含正则表达式模式和对应的关节角度
            
        Returns:
            torch.Tensor: 应用配置后的关节位置 [N, 12]
        """
        import re
        ...
        # 开始时使用随机化的默认关节位置
        joint_positions = self._randomized_default_joint_pos[env_ids].clone()
        
        # 应用关节位置配置
        for pattern, value in joint_config.items():
            # 找到匹配的关节
            matching_indices = []
            ...
            if matching_indices:
                for joint_idx in matching_indices:
                    joint_positions[:, joint_idx] = value
