...
@configclass
class MitFlatEnvCfg(DirectRLEnvCfg):    # env
    episode_length_s =20
    decimation = 4
    action_space = 12
    ...
    observation_space = 50  # 3(vel_cmd) + 2(orientation_cmd) + 1(height_cmd) + 3(imu_ang) + 3(imu_acc) + 3(proj_grav) + 12(joint_pos) + 12(joint_vel) + 12(actions) = 50
    state_space = 0
    ...
    upward_reward_scale = 4    # joint_pos_limits_reward_scale = -1.0
    joint_vel_reward_scale = -1e-3    stand_still = 2
    
    lin_vel_reward_scale = 1.5
    yaw_rate_reward_scale = 0.75
    pitch_reward_scale = 0.5  # 新增：pitch角度跟踪奖励尺度
    roll_reward_scale = 0.5   # 新增：roll角度跟踪奖励尺度
    height_reward_scale = 1.0 # 新增：身体高度跟踪奖励尺度 (增加到1.0)
...
class MitRoughEnvCfg(MitFlatEnvCfg):
    # env    #observation_space = 48 + 64
    observation_space = 52  # 50(基础观察) + 2(额外传感器数据) = 52
    terrain = TerrainImporterCfg(
        prim_path="/World/ground",
