...
# Pre-defined configs
##
from isaaclab_assets.robots.mit import Mit_CFG # isort: skip
...
class MitFlatEnvCfg(DirectRLEnvCfg):
    ...
    joint_power_reward_scale: float = -2e-5
    joint_pos_deviation_scale = -0.02 # 惩罚关节位置偏离
    feet_slide_penalty_scale: float = -0.0  # 脚滑移惩罚权重（身体坐标系）
    feet_stumble_penalty_scale: float = -0.1  # 脚部撞击垂直表面惩罚权重
    contact_forces_penalty_scale: float = -1.5e-4  # 接触力惩罚权重
    adaptive_gait_reward_scale: float = 0.2  # 适应性步态奖励权重
    obstacle_recovery_reward_scale: float = 0.5  # 障碍物恢复奖励权重
    proactive_stepping_reward_scale: float = 0.3  # 主动步态奖励权重
    terrain_adaptation_reward_scale: float = 0.4  # 地形适应奖励权重
    proprioceptive_prediction_reward_scale: float = 0.2  # 本体感知预测奖励权重
    feet_clearance_reward_scale: float = 0.0  # 脚离地高度奖励权重 
    feet_height_reward_scale: float = -0  # 摆动脚高度奖励权重
    stand_still_penalty_scale: float = -0.3  # 站立静止时偏离默认姿态的惩罚权重
