...
class MitEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MitFlatEnvCfg | MitRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._init_pose_variants = {
            ...
            "walking_r": {
                "F[L,R]_hip_joint": 0.0,
                "R[L,R]_hip_joint": -0.0,
                "FL_thigh_joint": -52 * torch.pi / 180.0,
                "FL_calf_joint": 94 * torch.pi / 180.0,
                "FR_thigh_joint": -11 * torch.pi / 180.0,
                "FR_calf_joint": 94 * torch.pi / 180.0,
                "RL_thigh_joint": 14 * torch.pi / 180.0,
                "RL_calf_joint": -82 * torch.pi / 180.0,
                "RR_thigh_joint": 46* torch.pi / 180.0,
                "RR_calf_joint": -82 * torch.pi / 180.0,
        }
        }
        ...
        self._episode_sums = {
            ...
            for key in [
                ...
                "feet_slide_penalty",  # 新增：脚滑移惩罚（身体坐标系）
                "feet_stumble_penalty",  # 新增：脚部撞击惩罚
                "contact_forces_penalty",  # 新增：接触力惩罚
        ...
        # 初始化所有环境的随机命令（10%概率为零命令）
        zero_command_prob = 0.1
        zero_mask = torch.rand(self.num_envs, device=self.device) < zero_command_prob
        
        # 生成随机命令
        x_commands = torch.empty(self.num_envs, device=self.device).uniform_(-1.0, 1.0)
        y_commands = torch.empty(self.num_envs, device=self.device).uniform_(-1.0, 1.0)
        yaw_commands = torch.empty(self.num_envs, device=self.device).uniform_(-1.0, 1.0)
        
        # 应用零命令掩码
        self._commands[:, 0] = torch.where(zero_mask, torch.zeros_like(x_commands), x_commands)  # x速度
        self._commands[:, 1] = torch.where(zero_mask, torch.zeros_like(y_commands), y_commands)  # y速度  
        self._commands[:, 2] = torch.where(zero_mask, torch.zeros_like(yaw_commands), yaw_commands)  # yaw速度
    ...
    def _compute_joint_pos_deviation_penalty(self) -> torch.Tensor:
        ...
        """
        # 设置参数
        stand_still_scale = 3.0  # 静止时的惩罚放大倍数
        ...
        default_joint_pos = self._robot.data.default_joint_pos  # [N, 12]
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # 为重置的环境重新采样速度和高度命令（10%概率为零命令）
        num_reset = len(env_ids)
        zero_command_prob = 0.1
        zero_mask = torch.rand(num_reset, device=self.device) < zero_command_prob
        
        # 生成随机命令
        x_commands = torch.empty(num_reset, device=self.device).uniform_(-1.0, 1.0)
        y_commands = torch.empty(num_reset, device=self.device).uniform_(-1.0, 1.0)
        yaw_commands = torch.empty(num_reset, device=self.device).uniform_(-1.0, 1.0)
        
        # 应用零命令掩码
        self._commands[env_ids, 0] = torch.where(zero_mask, torch.zeros_like(x_commands), x_commands)  # x速度
        self._commands[env_ids, 1] = torch.where(zero_mask, torch.zeros_like(y_commands), y_commands)  # y速度  
        self._commands[env_ids, 2] = torch.where(zero_mask, torch.zeros_like(yaw_commands), yaw_commands)  # yaw速度
    ...
    def _update_command_resample(self):
        ...
        if torch.any(resample_mask):
            ...
            # 10%概率采样零命令（静止状态）
            zero_command_prob = 0.1
            zero_mask = torch.rand(num_resample, device=self.device) < zero_command_prob
            
            # 重新采样速度命令 (x, y, yaw)
            x_commands = torch.empty(num_resample, device=self.device).uniform_(-1.0, 1.0)
            y_commands = torch.empty(num_resample, device=self.device).uniform_(-1.0, 1.0)  
            yaw_commands = torch.empty(num_resample, device=self.device).uniform_(-1.0, 1.0)
            
            # 应用零命令掩码：10%概率设为0，90%概率使用随机值
            self._commands[resample_env_ids, 0] = torch.where(zero_mask, torch.zeros_like(x_commands), x_commands)  # x速度
            self._commands[resample_env_ids, 1] = torch.where(zero_mask, torch.zeros_like(y_commands), y_commands)  # y速度  
            self._commands[resample_env_ids, 2] = torch.where(zero_mask, torch.zeros_like(yaw_commands), yaw_commands)  # yaw速度
            
            # 重新采样高度命令（高度命令不受零命令掩码影响）
