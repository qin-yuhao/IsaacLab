...
class MitEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: MitFlatEnvCfg | MitRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 命令重新采样相关变量
        self._command_resample_time_min = 5  # 最小重新采样间隔（秒）
        self._command_resample_time_max = 20.0  # 最大重新采样间隔（秒）
        ...
        if num_random > 0:
            # 随机命令：完全随机采样
            initial_commands[random_mask, 0] = torch.empty(num_random, device=self.device).uniform_(-3.0, 3.0)  # x速度
    ...
    def _apply_action(self):
        ...
        pos_target[:, 12:] = 0.0  # 将后4个关节位置置为0

        vel_target = self._processed_actions.clone() #置0前12
        vel_target[:, :12] = 0.0

        self._robot.set_joint_position_target(pos_target)  # 前12个关节的目标位置
        # vel joint target  后4个
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        joint_acc_legs = joint_acc_full[:, :8]  # [N, 8] - 前8个关节（腿部关节）
        joint_acc_wheels = joint_acc_full[:, 8:]  # [N, 4] - 后4个关节（轮子关节）
    ...
    def _compute_joint_pos_deviation_penalty(self) -> torch.Tensor:
        ...
        default_joint_pos = self._robot.data.default_joint_pos[:, :12]  # [N, 12] - 只取前12个关节
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if num_random > 0:
            # 使用固定的速度范围
            x_vel_max, y_vel_max, yaw_vel_max = 3.0, 1.0, 1.0
    ...
    def _update_command_resample(self):
        ...
        if torch.any(resample_mask):
            ...
            if num_random > 0:
                # 随机命令：完全随机采样
                new_commands[random_mask, 0] = torch.empty(num_random, device=self.device).uniform_(-1.0, 1.0)  # x速度
                new_commands[random_mask, 1] = torch.empty(num_random, device=self.device).uniform_(-1.0, 1.0)  # y速度
                new_commands[random_mask, 2] = torch.empty(num_random, device=self.device).uniform_(-1.0, 1.0)  # yaw速度
    ...
    def _compute_wheel_velocity_penalty(self) -> torch.Tensor:
        ...
        # 获取后4个关节（轮子关节）的速度
        wheel_joint_vel = torch.abs(self._robot.data.joint_vel[:, 8:12])  # [N, 4] - 后4个关节
