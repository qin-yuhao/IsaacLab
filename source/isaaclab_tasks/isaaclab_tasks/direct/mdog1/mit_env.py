...
class MitEnv(DirectRLEnv):
    ...
    def _apply_pose_variant_to_joint_positions(self, env_ids: torch.Tensor, pose_variant: str) -> torch.Tensor:
        """将姿态变体应用到关节位置
        
        Args:
            env_ids: 环境ID
            pose_variant: 姿态变体名称 ("standing", "lie_down", "walking")
            
        Returns:
            torch.Tensor: 应用姿态变体后的关节位置 [N, 12]
        """
        # 获取机器人的关节名称
        ...
        # 获取指定姿态变体的配置
        if pose_variant not in self._init_pose_variants:
            raise ValueError(f"Unknown pose variant: {pose_variant}")
        
        pose_config = self._init_pose_variants[pose_variant]
        
        # 应用姿态变体到关节位置
        import re
        for pattern, value in pose_config.items():
            # 找到匹配的关节
            matching_indices = []
