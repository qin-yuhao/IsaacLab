...
@configclass
class MitRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()

        #self.scene.robot = UNITREE_A1_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        self.scene.robot = Mit_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        ...
        self.events.reset_robot_joints.params["position_range"] = (0, 2.0)
...
@configclass
class MitRoughEnvCfg_PLAY(MitRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
